/*==============================================================================
  COUNTERFACTUAL 2: GENDER WAGE-GAP CHANNEL

  Question: "What if the labour market rewarded women's human capital the
             same way it rewards men's?"

  Method:   Recompute every wife's permanent income using the MALE Mincer
            coefficients, keeping her actual age at marriage.
...
di _n "=== Counterfactual 2: Using MALE coefficients for wife ==="
...
* --- 5a. Wife's NPV using MALE coefficients, wife's actual marriage age ---
...
replace wife_NPV_cf = . if missing(wife_eduyear) | missing(wife_marriage_age)
replace wife_NPV_cf = . if wife_marriage_age > `retire_age'
label var wife_NPV_cf "Wife's CF2 PDV (male coefficients, own marriage age)"
...
label var wife_yp_cf "Wife's CF2 permanent income (male coefficients)"
...
di _n "=== CF2 permanent income computed ==="
count
...
label var diff_yp_cf "CF2: Wife - Husband permanent income (yuan)"
...
label var pct_diff_cf "CF2: |(wife-husband) yp| / couple avg yp"
drop _couple_avg

* Mutually exclusive categories (5% threshold)
gen homogamy_cf = (pct_diff_cf < 0.05) ///
    if !missing(pct_diff_cf)
label var homogamy_cf "CF2: 1 = Perm income within 5% of couple avg"

gen hypergamy_cf = (pct_diff_cf >= 0.05 & husband_yp > wife_yp_cf) ///
    if !missing(pct_diff_cf)
label var hypergamy_cf "CF2: 1 = Husband perm income >5% above couple avg"

gen hypogamy_cf = (pct_diff_cf >= 0.05 & wife_yp_cf > husband_yp) ///
    if !missing(pct_diff_cf)
label var hypogamy_cf "CF2: 1 = Wife perm income >5% above couple avg"
...
* --------------------------------------------------------------------------
* Chart 1: Average wife − husband permanent income gap (CF2)
* --------------------------------------------------------------------------
...
twoway (connected mean_diff marry_year, ///
    ...
    , ///
    title("CF2: Average Wife-Husband Permanent Income Gap") ///
    subtitle("Wife uses male coefficients, own marriage age") ///
...
graph export "${output}/cf2_trend_diff_yp.png", replace width(1200)

restore


* --------------------------------------------------------------------------
* Chart 2: Stacked bar — % hypergamy / hypogamy / homogamy (CF2)
* --------------------------------------------------------------------------
...
twoway (rbar hyper_bot hyper_top marry_year, ///
    ...
    , ///
    title("CF2: Hypergamy / Homogamy / Hypogamy by Marriage Year") ///
    subtitle("Wife uses male coefficients, own marriage age (5% threshold)") ///
    ...
    scheme(s2color)
graph export "${output}/cf2_bar_hypergamy.png", replace width(1200)

restore


* --------------------------------------------------------------------------
* Chart 3: CF2 vs Reality — average wife-husband permanent income gap
* --------------------------------------------------------------------------
...
merge 1:1 marry_year using `base_cf2', nogen
...
twoway (connected mean_diff_real marry_year, ///
    ...
    , ///
    title("Wife-Husband Permanent Income Gap: Reality vs CF2 vs CF3") ///
    subtitle("CF2: No gender wage gap; CF3: No wage gap + no age gap") ///
    ytitle("Mean difference (yuan/year)") xtitle("Marriage year") ///
    ylabel(, format(%12.0fc)) ///
    xlabel(2010(1)2020, angle(45)) ///
    yline(0, lcolor(gs8) lpattern(dash) lwidth(thin)) ///
    legend(order(1 "Reality (gender-specific coefficients)" ///
                 2 "CF2: No gender wage gap" ///
                 3 "CF3: No wage gap + no age gap") ///
           position(6) rows(1) size(small)) ///
    note("Below 0 = hypergamy (husband earns more)" ///
         "Above 0 = hypogamy (wife earns more)") ///
    scheme(s2color)
graph export "${output}/cf2_vs_reality_trend_diff_yp.png", replace width(1200)
...
compress
save "${work}/cfps_counterfactual_2_wage_gap.dta", replace

di _n "================================================================="
di    "DONE — Counterfactual 2: Gender Wage-Gap Channel"
di    "  Wife's y^p recomputed with MALE coefficients,"
di    "  keeping her actual marriage age."
di    "  Saved: ${work}/cfps_counterfactual_2_wage_gap.dta"
di    "  Charts: ${output}/cf2_*.png"
di    "================================================================="
tab marry_year
count
